...
ggplot(mpg, aes(x=cty, y=hwy)) +      # positional argument
...
#ggplot(mapping=aes(x=cty, y=hwy), data=mpg) +   # keyword argument
#  geom_point() 
...
View(diamonds)
sort_diamonds <- arrange(diamonds,cut,color)
ggplot(diamonds,aes(x=cut,y=price,fill=color)) +
  geom_bar(stat="identity") 
   #geom_text(aes(y=label,label=paste(color,"color")), colour="black",size=4)
